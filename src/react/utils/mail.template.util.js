export function CreateTemplate (templateName, data) {
    ...
    switch(templateName) {
        ...
        case 'reminderModule':
            ...
            body = `body=Hallo ${data.titel} ${data.profLastName},%0D%0A%0D%0A Am ${data.date} habe ich Sie um die Prüfung eines Moduls für den Fall von ${data.gender} ${data.caseLastName} gebeten, jedoch bisher keine Rückmeldung erhalten.%0D%0A damit ich den Fall abschließen kann, bitte ich um ihre Rückmeldung.%0D%0A`
            return subject + body
        default: return ''
    }
}
